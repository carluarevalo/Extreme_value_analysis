...
```{r}
mu    <- 0.03636916#gum$mle[1]
sigma <- 0.98582371#gum$mle[2]
```
...
```{r}
plot(estims, logyps, xlab=expression(z[p]), ylab=expression(log(y[p])))
```
...
Utilice las estimaciónes que devuelve `gum.fit` junto con los gradientes de los estimadores
